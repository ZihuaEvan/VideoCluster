...
def test_retrieval(model, criterion, transforms_cuda, device, epoch, args):
    ...
    with torch.no_grad():
        ...
        if os.path.exists(os.path.join(os.path.dirname(args.test), dirname, '%s_train_feature.pth.tar' % args.dataset)):
            ...
            #  print(train_label)
            feature_cluster = train_feature.cpu().numpy()
            estimator = KMeans(n_clusters=10)
            estimator.fit(np.array(feature_cluster))
            y_pred = estimator.predict(feature_cluster)
            cluster_label = [1]*100+[2]*100+[3]*100+[4]*100+[5]*100+[6]*100+[7]*100+[8]*100+[9]*100+[10]*100
            accuracy = cal_accuracy(cluster_label, y_pred)
            print('Initial Accuracy: {}'.format(accuracy))
            dst = 'result/data.txt'
            fp = open(dst, "a", encoding="utf-8")
            fp.write('_clip_1' +
                     'acc: ' + str(accuracy) + '\n')
            fp.close()
